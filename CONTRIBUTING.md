...
当前步骤 1 的实现和完整发布门禁已经通过，状态为待全新严格评审。步骤 2 至步骤 7 均未开始，不得把后续目标架构写成当前已经实现的能力。
...
src/sim/metrics     旧固定指标兼容实现，步骤 2、3、5 降为状态与观察投影
src/sim/timeline    旧预制纪元事件兼容实现，步骤 2 改为状态差异产生历史
src/sim/galaxies    旧传统空间样本兼容实现，步骤 2、5 改为持续状态与动态拓扑
...
src/sim/universe    旧兼容懒物化入口与产品强制因果入口
src/App.tsx         当前网页应用装配入口
tests/              自动化验收测试
docs/               当前事实来源与历史文档归档
```
...
- 以下约束只保护旧基线兼容路径，不是目标架构：局部对象暂时消费 `UniverseSummary.timelineImpact`，文明暂时从 `Biosphere.civilizationSeed` 派生，固定奇迹暂时使用 `InterventionInput[]`、`TargetMutation` 与 I1，A1 暂时通过 `archiveStorage.ts` 和版本化分享码恢复。它们分别由步骤 2、4、5、6、7 按迁移矩阵退出，禁止复制到新运行时。
