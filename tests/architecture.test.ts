...
describe("架构边界门禁", () => {
  ...
  it("关键编排文件保持在约定规模内", () => {
    const limits = new Map([
      ...
      ["src/components/RuntimeNavigation.tsx", 60],
      ["src/components/RuntimeSession.tsx", 70],
      ["src/components/LegacyApplication.tsx", 100],
      ["src/ui/useRuntimeUniverseModel.ts", 180],
      ["src/ui/runtimeStorage.ts", 130],
      ["src/ui/useObservationWorkbench.ts", 170],
      ["src/ui/researchStorage.ts", 120],
      ["src/ui/researchNotebookLoader.ts", 50],
      ["src/ui/useBranchLaboratory.ts", 160],
      ["src/ui/branchStorage.ts", 120],
      ["src/sim/contracts/runtime.ts", 190],
      ["src/sim/contracts/observation.ts", 150],
      ["src/sim/observation.ts", 130],
      ["src/sim/research-archive.ts", 120],
      ["src/sim/research-migration.ts", 100],
      ["src/sim/contracts/branching.ts", 110],
      ["src/sim/branching.ts", 130],
      ["src/sim/branch-replay.ts", 50],
      ["src/sim/branch-comparison.ts", 90],
      ["src/sim/branch-archive.ts", 60],
      ["src/sim/branch-packages.ts", 70],
      ["src/sim/branch-inputs.ts", 70],
      ...
      ["src/components/pages/ObservationPage.tsx", 90],
      ["src/components/pages/ResearchPage.tsx", 60],
      ["src/components/pages/ExperimentPage.tsx", 70],
      ["src/components/pages/BranchesPage.tsx", 70],
    ...
    const styleLimits = new Map([
      ...
      ["src/styles-observation.css", 100],
